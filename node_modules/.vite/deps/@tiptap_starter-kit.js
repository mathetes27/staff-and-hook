import { $t as markPasteRule, An as Plugin, Bt as isNodeActive, Cn as textblockTypeInputRule, Dn as Decoration, E as combineTransactionSteps, En as keydownHandler, Fn as dropPoint, G as findChildrenInRange, Ht as isNodeSelection, In as Fragment, Ln as Slice, Mn as Selection$1, Mt as isAtStartOfNode, Nn as TextSelection, On as DecorationSet, Pn as Mapping, Qt as markInputRule, T as canInsertNode, Tn as wrappingInputRule, Vt as isNodeEmpty, an as nodeInputRule, et as getAttributes, f as Mark, ft as getNodeType, it as getExtensionField, jn as PluginKey, jt as isAtEndOfNode, kn as NodeSelection, ln as parseIndentedBlocks, lt as getMarksBetween, m as Node3, mt as getRenderedAttributes, nn as mergeAttributes, nt as getChangedRanges, pn as renderNestedMarkdownContent, pt as getPreviousBlockSibling, s as Extension, u as InputRule, ut as getNodeAtPosition, v as PasteRule, w as callOrReturn } from "./dist-CN5VY376.js";
...
function isConvertibleLink(text, match, isAllowedHref) {
	...
	if ((match.index ? text[match.index - 1] : void 0) === "!" || isEscaped(text, (_a = match.index) != null ? _a : 0)) return false;
	if (isInsideCodeSpan(text, (_b = match.index) != null ? _b : 0)) return false;
...
function markdownLinkPasteRule(config) {
	const rule = markPasteRule({
		find: (text) => {
			...
			const plainUrlMatches = ((_b = (_a = config.findPlainUrls) == null ? void 0 : _a.call(config, text)) != null ? _b : []).filter((urlMatch) => !markdownMatches.some((markdownMatch) => matchesOverlap(markdownMatch, urlMatch)));
	...
	return new PasteRule({
		...
		handler: (props) => {
			var _a;
			const result = rule.handler(props);
			if (result !== null && props.state.tr.steps.length && ((_a = props.match.data) == null ? void 0 : _a.markdown)) props.state.tr.setMeta("preventAutolink", true);
			return result;
		}
	});
}
...
var Link = Mark.create({
	...
	addAttributes() {
		...
		return {
			...
			target: { default: (_a = this.options.HTMLAttributes.target) != null ? _a : null },
			rel: { default: (_b = this.options.HTMLAttributes.rel) != null ? _b : null },
			class: { default: (_c = this.options.HTMLAttributes.class) != null ? _c : null },
			title: { default: null }
		};
	},
	parseHTML() {
	...
	renderMarkdown: (node, h, ctx) => {
		return renderNestedMarkdownContent(node, h, (context) => {
			...
			if (context.parentType === "orderedList") {
				...
				return getListMarker((_d = (_c = context.meta) == null ? void 0 : _c.parentAttrs) == null ? void 0 : _d.type, start - 1 + (context.index || 0), ". ");
			}
			return "- ";
		}, ctx);
	},
...
var OrderedList = Node3.create({
	...
	markdownTokenizer: {
		...
		tokenize: (src, _tokens, lexer) => {
			...
			return {
				...
				typeMarker: (_b = listItems[0]) == null ? void 0 : _b.type,
				items,
	...
	addProseMirrorPlugins() {
		return [new Plugin({ props: { handlePaste: (view, event) => {
			...
			if (html == null ? void 0 : html.trim()) return false;
...
var TaskItem = Node3.create({
	...
	renderMarkdown: (node, h) => {
		var _a;
		const prefix = `- [${((_a = node.attrs) == null ? void 0 : _a.checked) ? "x" : " "}] `;
...
var Gapcursor = Extension.create({
	...
	extendNodeSchema(extension) {
		...
		return { allowGapCursor: (_a = callOrReturn(getExtensionField(extension, "allowGapCursor", context))) != null ? _a : null };
	}
});
