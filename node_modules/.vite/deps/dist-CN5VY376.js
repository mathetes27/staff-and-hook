...
var NodeViewDesc = class NodeViewDesc extends ViewDesc {
	...
	static create(parent, node, outerDeco, innerDeco, view, pos) {
		...
		if (spec) return descObj = new CustomNodeViewDesc(parent, node, outerDeco, innerDeco, dom, contentDOM || null, nodeDOM, spec);
		else if (node.isText) return new TextViewDesc(parent, node, outerDeco, innerDeco, dom, nodeDOM);
		else return new NodeViewDesc(parent, node, outerDeco, innerDeco, dom, contentDOM || null, nodeDOM);
	}
	parseRule(addedNodes) {
...
function docViewDesc(doc, outerDeco, innerDeco, dom, view) {
	...
	let docView = new NodeViewDesc(void 0, doc, outerDeco, innerDeco, dom, dom, dom);
...
function selectionForTripleClick(view, inside, selectNodes) {
	...
	for (let i = $pos.depth + 1; i > 0; i--) {
		...
		if (node.inlineContent) return TextSelection.create(doc, nodePos + 1, nodePos + 1 + node.content.size);
		else if (selectNodes && NodeSelection.isSelectable(node)) return NodeSelection.create(doc, nodePos);
	}
	return null;
}
...
handlers.beforeinput = (view, _event) => {
	if (android && _event.inputType == "deleteContentBackward") {
...
*/
var Decoration$1 = class Decoration$1 {
	/**
	@internal
	*/
	...
	*/
	static widget(pos, toDOM, spec) {
		return new Decoration$1(pos, pos, new WidgetType(toDOM, spec));
	}
	/**
	...
	*/
	static node(from, to, attrs, spec) {
		return new Decoration$1(from, to, new NodeType(attrs, spec));
	}
	/**
...
var ruleFromNode = (added) => (dom) => {
	let desc = dom.pmViewDesc;
	if (desc) return desc.parseRule(added);
...
function updateCursorWrapper(view) {
	if (view.markCursor) {
		...
		view.cursorWrapper = {
			dom,
			deco: Decoration$1.widget(view.state.selection.from, dom, {
				raw: true,
...
var joinListBackwards = (tr, listType) => {
	...
	if (!areListTypesCompatible(list.node.attrs.type, nodeBefore == null ? void 0 : nodeBefore.attrs.type)) return true;
	tr.join(list.pos);
	return true;
};
var joinListForwards = (tr, listType) => {
	...
	if (!areListTypesCompatible(list.node.attrs.type, nodeAfter == null ? void 0 : nodeAfter.attrs.type)) return true;
	tr.join(after);
	return true;
};
...
function validateDecorationSpec(name, spec) {
	var _a;
	switch ((_a = spec.update) != null ? _a : "document") {
		case "document":
...
var ExtensionManager = class {
	...
	*/
	createDecorationPlugin() {
		var _a;
		const { editor } = this;
		(_a = this.decorationManager) == null || _a.destroy();
		const entries = [];
...
function createWidgetDecoration(options) {
	...
	return Decoration.Widget(pos, render, {
		...
		destroy: (rendererElement) => {
			...
			try {
				(_a2 = cache.renderers.get(key)) == null || _a2.destroy();
...
//#endregion
export { generateText as $, markPasteRule as $t, createChainableState as A, Plugin as An, isAndroid as At, elementFromString as B, isNodeActive as Bt, attrsEqual as C, textblockTypeInputRule as Cn, getTextContentFromNodes as Ct, commands_exports as D, Decoration$1 as Dn, injectExtensionAttributesToParseRule as Dt, combineTransactionSteps as E, keydownHandler as En, h as Et, createStyleTag as F, dropPoint as Fn, isFirefox as Ft, findChildrenInRange as G, isPlainObject as Gt, escapeForRegEx as H, isNodeSelection as Ht, createWidgetDecoration as I, Fragment as In, isFunction as It, findParentNodeClosestToPos as J, isString as Jt, findDuplicates as K, isRegExp as Kt, decodeHtmlEntities as L, Slice as Ln, isList as Lt, createInlineMarkdownSpec as M, Selection as Mn, isAtStartOfNode as Mt, createMappablePosition as N, TextSelection as Nn, isEmptyObject as Nt, createAtomBlockMarkdownSpec as O, DecorationSet as On, inputRulesPlugin as Ot, createNodeFromContent as P, Mapping as Pn, isExtensionRulesEnabled as Pt, generateJSON as Q, markInputRule as Qt, defaultBlockAt as R, isMacOS as Rt, WidgetDecoration as S, textPasteRule as Sn, getTextBetween as St, canInsertNode as T, wrappingInputRule as Tn, getUpdatedPosition as Tt, extensions_exports as U, isNodeViewSelected as Ut, encodeHtmlEntities as V, isNodeEmpty as Vt, findChildren as W, isNumber as Wt, fromString as X, isiOS as Xt, flattenExtensions as Y, isTextSelection as Yt, generateHTML as Z, liveWidgetKeys as Zt, NodeView as _, selectionToInsertionEnd as _n, getSchemaTypeByName as _t, Editor as a, nodeInputRule as an, getHTMLFromFragment as at, ResizableNodeview as b, splitExtensions as bn, getStyleProperty as bt, Fragment7 as c, parseAttributes as cn, getMarkType as ct, MappablePosition as d, posToDOMRect as dn, getNodeAttributes as dt, markdown_exports as en, getAttributes as et, Mark as f, removeDuplicates as fn, getNodeType as ft, NodePos as g, rewriteUnknownContent as gn, getSchemaByResolvedExtensions as gt, NodeDecoration as h, resolveFocusPosition as hn, getSchema as ht, DecorationManager as i, minMax as in, getExtensionField as it, createDocument as j, PluginKey as jn, isAtEndOfNode as jt, createBlockMarkdownSpec as k, NodeSelection as kn, isActive as kt, InlineDecoration as l, parseIndentedBlocks as ln, getMarksBetween as lt, Node3 as m, resolveExtensions as mn, getRenderedAttributes as mt, DECORATION_MANAGER_PLUGIN_KEY as n, mergeAttributes as nn, getChangedRanges as nt, Extendable as o, nodePasteRule as on, getMarkAttributes as ot, MarkView as p, renderNestedMarkdownContent as pn, getPreviousBlockSibling as pt, findParentNode as q, isSafari as qt, Decoration as r, mergeDeep as rn, getDebugJSON as rt, Extension as s, objectIncludes as sn, getMarkRange as st, CommandManager as t, marksEqual as tn, getAttributesFromExtensions as tt, InputRule as u, pasteRulesPlugin as un, getNodeAtPosition as ut, PasteRule as v, serializeAttributes as vn, getSchemaTypeNameByName as vt, callOrReturn as w, updateMarkViewAttributes as wn, getTextSerializersFromSchema as wt, Tracker as x, textInputRule as xn, getText as xt, ResizableNodeView as y, sortExtensions as yn, getSplittedAttributes as yt, deleteProps as z, isMarkActive as zt };
