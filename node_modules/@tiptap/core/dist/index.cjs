...
var deleteSelection = () => ({ state, dispatch }) => {
  ...
  if (dispatch) {
    ...
    if (!tr.selection.empty) {
      tr.setSelection(import_state2.TextSelection.near(tr.doc.resolve(tr.selection.from)));
    }
    tr.scrollIntoView();
    dispatch(tr);
  }
...
var exitCode = () => ({ state, dispatch }) => {
  return (0, import_commands2.exitCode)(state, dispatch);
};
...
var extendMarkRange = (typeOrName, attributes) => ({ tr, state, dispatch }) => {
  ...
  if (dispatch) {
    ...
    if (range && range.from <= from && range.to >= to) {
      const newSelection = import_state3.TextSelection.create(doc, range.from, range.to);
...
function isTextSelection(value) {
  return value instanceof import_state4.TextSelection;
}
...
function resolveFocusPosition(doc, position = null) {
  ...
  const selectionAtStart = import_state5.Selection.atStart(doc);
  const selectionAtEnd = import_state5.Selection.atEnd(doc);
  ...
  if (position === "all") {
    return import_state5.TextSelection.create(
      doc,
...
// src/helpers/isProseMirrorContent.ts
...
function createNodeFromContent(content, schema, options) {
  ...
  if (isTextContent) {
    if (options.errorOnInvalidContent) {
      ...
      if (options.slice) {
        import_model.DOMParser.fromSchema(contentCheckSchema).parseSlice(
          elementFromString(content),
          options.parseOptions
        );
      } else {
        import_model.DOMParser.fromSchema(contentCheckSchema).parse(
          elementFromString(content),
          options.parseOptions
        );
      }
...
function selectionToInsertionEnd(tr, startLen, bias) {
  ...
  tr.setSelection(import_state6.Selection.near(tr.doc.resolve(end), bias));
}
...
var insertContentAt = (position, value, options) => ({ tr, dispatch, editor }) => {
  ...
  if (dispatch) {
    ...
    if (isOnlyTextContent) {
      ...
      newContent = import_model2.Fragment.from(nodes);
...
var joinUp = () => ({ state, dispatch }) => {
  return (0, import_commands3.joinUp)(state, dispatch);
};
var joinDown = () => ({ state, dispatch }) => {
  return (0, import_commands3.joinDown)(state, dispatch);
};
var joinBackward = () => ({ state, dispatch }) => {
  return (0, import_commands3.joinBackward)(state, dispatch);
};
var joinForward = () => ({ state, dispatch }) => {
  return (0, import_commands3.joinForward)(state, dispatch);
};
...
var joinTextblockBackward = () => ({ state, dispatch }) => {
  return (0, import_commands4.joinTextblockBackward)(state, dispatch);
};

// src/commands/joinTextblockForward.ts
var import_commands5 = require("@tiptap/pm/commands");
var joinTextblockForward = () => ({ state, dispatch }) => {
  return (0, import_commands5.joinTextblockForward)(state, dispatch);
};
...
var lift = (typeOrName, attributes = {}) => ({ state, dispatch }) => {
  ...
  return (0, import_commands6.lift)(state, dispatch);
};

// src/commands/liftEmptyBlock.ts
var import_commands7 = require("@tiptap/pm/commands");
var liftEmptyBlock = () => ({ state, dispatch }) => {
  return (0, import_commands7.liftEmptyBlock)(state, dispatch);
};
...
var newlineInCode = () => ({ state, dispatch }) => {
  return (0, import_commands8.newlineInCode)(state, dispatch);
};
...
var selectAll = () => ({ tr, dispatch }) => {
  if (dispatch) {
    const selection = new import_state7.AllSelection(tr.doc);
    tr.setSelection(selection);
  }
  return true;
};
...
// src/helpers/getPreviousBlockSibling.ts
...
function isNodeSelection(value) {
  return value instanceof import_state8.NodeSelection;
}
...
var setNode = (typeOrName, attributes = {}) => ({ state, dispatch, chain }) => {
  ...
  return chain().command(({ commands }) => {
    const canSetBlock = (0, import_commands14.setBlockType)(type, { ...attributesToCopy, ...attributes })(state);
    if (canSetBlock) {
      return true;
    }
    return commands.clearNodes();
  }).command(({ state: updatedState }) => {
    return (0, import_commands14.setBlockType)(type, { ...attributesToCopy, ...attributes })(updatedState, dispatch);
  }).run();
};
...
var setNodeSelection = (position) => ({ tr, dispatch }) => {
  if (dispatch) {
    ...
    const selection = import_state9.NodeSelection.create(doc, from);
    tr.setSelection(selection);
  }
  return true;
};
...
var setTextSelection = (position) => ({ tr, dispatch }) => {
  if (dispatch) {
    ...
    const minPos = import_state10.TextSelection.atStart(doc).from;
    const maxPos = import_state10.TextSelection.atEnd(doc).to;
    const resolvedFrom = minMax(from, minPos, maxPos);
    const resolvedEnd = minMax(to, minPos, maxPos);
    const selection = import_state10.TextSelection.create(doc, resolvedFrom, resolvedEnd);
    tr.setSelection(selection);
  }
  return true;
};
...
var splitBlock = ({ keepMarks = true } = {}) => ({ tr, state, dispatch, editor }) => {
  ...
  if (selection instanceof import_state11.NodeSelection && selection.node.isBlock) {
  ...
  if (dispatch) {
    if (can) {
      if (selection instanceof import_state11.TextSelection) {
        tr.deleteSelection();
      }
...
var splitListItem = (typeOrName, overrideAttrs = {}) => ({ tr, state, dispatch, editor }) => {
  ...
  if ($from.parent.content.size === 0 && $from.node(-1).childCount === $from.indexAfter(-1)) {
    ...
    if (dispatch) {
      ...
      if (sel > -1) {
        tr.setSelection(import_state12.TextSelection.near(tr.doc.resolve(sel)));
      }
      tr.scrollIntoView();
    }
    return true;
...
function createInnerSelectionForWholeDocList(tr) {
  ...
  return import_state13.TextSelection.between($start, $end);
}
...
var wrapIn = (typeOrName, attributes = {}) => ({ state, dispatch }) => {
  const type = getNodeType(typeOrName, state.schema);
  return (0, import_commands15.wrapIn)(type, attributes)(state, dispatch);
};
...
// src/decorations/decorationApplyScope.ts
...
// src/utilities/isDev.ts
var isDev = typeof process !== "undefined" && process.env.NODE_ENV !== "production";

// src/decorations/helpers/buildDecorationSet.ts
var import_view = require("@tiptap/pm/view");

// src/decorations/helpers/decorationsToPMDecorations.ts
...
function buildDecorationSet(doc, decorations, extensionName) {
  ...
  return { set: import_view.DecorationSet.create(doc, pmDecorations), widgetKeys };
}

// src/decorations/helpers/rangeOwnsPosition.ts
...
// src/decorations/helpers/filterOutOfRangeDecorations.ts
...
// src/decorations/helpers/widgetKeyOf.ts
...
// src/decorations/helpers/findDuplicateWidgetKeys.ts
...
// src/decorations/helpers/isAttrStep.ts
function isAttrStep(step) {
  return step.jsonID === "attr";
}

// src/decorations/helpers/hasResolvableChangedRange.ts
...
// src/decorations/helpers/getRebuildRanges.ts
...
// src/decorations/helpers/mapDecorations.ts
var import_view2 = require("@tiptap/pm/view");

// src/decorations/helpers/mapDecorationSet.ts
...
function mapDecorations(name, previous, tr) {
  var _a, _b;
  const previousSet = (_a = previous.decorationSetsByExtension[name]) != null ? _a : import_view2.DecorationSet.empty;
...
// src/decorations/helpers/mergeDecorationSets.ts
...
function mergeDecorationSets(doc, decorationSetsByExtension) {
  ...
  return import_view3.DecorationSet.create(doc, allDecorations);
}

// src/decorations/helpers/unionWidgetKeys.ts
...
// src/decorations/helpers/validateDecorationSpec.ts
...
// src/decorations/helpers/shouldRecomputeDecoration.ts
...
function liveWidgetKeys(editor) {
  var _a, _b, _c;
  return (_c = (_b = (_a = editor.extensionManager) == null ? void 0 : _a.decorationManager) == null ? void 0 : _b.liveWidgetKeys()) != null ? _c : EMPTY_KEYS;
}
var DecorationManager = class {
  ...
  createPlugin() {
    ...
    return new import_state15.Plugin({
      ...
      props: {
        decorations(state) {
          var _a, _b;
          return (_b = (_a = DECORATION_MANAGER_PLUGIN_KEY.getState(state)) == null ? void 0 : _a.mergedDecorationSet) != null ? _b : import_view4.DecorationSet.empty;
        }
      }
    });
  }
  ...
  rebuildRanges(name, spec, previous, tr, newState, ranges) {
    var _a, _b;
    const previousSet = (_a = previous.decorationSetsByExtension[name]) != null ? _a : import_view4.DecorationSet.empty;
    const widgetKeys = new Set((_b = previous.widgetKeysByExtension[name]) != null ? _b : []);
    let set = mapDecorationSet(previousSet, tr.mapping, tr.doc, widgetKeys);
    const docSize = newState.doc.content.size;
    for (const { from, to } of ranges) {
      const stale = set.find(from, to).filter((decoration) => rangeOwnsPosition({ position: decoration.from, from, to, docSize }));
...
function canInsertNode(state, nodeType) {
  ...
  if (selection instanceof import_state16.NodeSelection) {
...
function inputRulesPlugin(props) {
  ...
  const plugin = new import_state17.Plugin({
    state: {
      init() {
        return null;
      },
  ...
  addProseMirrorPlugins() {
    return [
      new import_state19.Plugin({
        key: new import_state19.PluginKey("clipboardTextSerializer"),
        props: {
  ...
  addProseMirrorPlugins() {
    return [
      new import_state20.Plugin({
        key: new import_state20.PluginKey("tiptapDrop"),
        props: {
  ...
  addProseMirrorPlugins() {
    return [
      new import_state21.Plugin({
        key: new import_state21.PluginKey("editable"),
        props: {
...
var focusEventsPluginKey = new import_state22.PluginKey("focusEvents");
  ...
  addKeyboardShortcuts() {
    const handleBackspace = () => this.editor.commands.first(({ commands }) => [
      ...
      () => commands.command(({ tr }) => {
        ...
        const isAtStart = parentIsIsolating && $parentPos.parent.childCount === 1 ? parentPos === $anchor.pos : import_state23.Selection.atStart(doc).from === pos;
  ...
  addProseMirrorPlugins() {
    return [
      ...
      new import_state23.Plugin({
        key: new import_state23.PluginKey("clearDocument"),
        appendTransaction: (transactions, oldState, newState) => {
          ...
          const allFrom = import_state23.Selection.atStart(oldState.doc).from;
          const allEnd = import_state23.Selection.atEnd(oldState.doc).to;
...
var Paste = Extension.create({
  ...
  addProseMirrorPlugins() {
    return [
      new import_state24.Plugin({
        key: new import_state24.PluginKey("tiptapPaste"),
        props: {
  ...
  addProseMirrorPlugins() {
    return [
      new import_state25.Plugin({
        key: new import_state25.PluginKey("tabindex"),
        props: {
          attributes: () => {
            var _a;
  ...
  addProseMirrorPlugins() {
    return [
      new import_state26.Plugin({
        key: new import_state26.PluginKey("textDirection"),
        props: {
          attributes: () => {
...
// src/helpers/warnOnDuplicatedProseMirrorModel.ts
var import_model11 = require("@tiptap/pm/model");
...
function warnOnDuplicatedProseMirrorModel(schema) {
  ...
  let content;
  try {
    content = import_transform10.ReplaceStep.fromJSON(schema, { from: 0, to: 0 }).slice.content;
  } catch {
    return;
  }
  if (content instanceof import_model11.Fragment) {
    return;
  }
  console.warn(
...
var Editor = class extends EventEmitter {
  constructor(options = {}) {
    ...
    if (!this.editorState) {
      ...
      this.editorState = import_state27.EditorState.create({
        doc: initialDoc,
        schema: this.schema,
  ...
  createView(element) {
    ...
    this.editorView = new import_view5.EditorView(element, {
      ...editorProps,
      attributes: {
...
var InlineDecoration = class extends Decoration {
  ...
  toPMDecoration(extensionName) {
    ...
    return import_view6.Decoration.inline(this.from, this.to, this.attrs, spec);
  }
};
var NodeDecoration = class extends Decoration {
  ...
  toPMDecoration(extensionName) {
    const spec = extensionName ? { ...this.spec, extensionName } : this.spec;
    return import_view6.Decoration.node(this.from, this.to, this.attrs, spec);
  }
};
var WidgetDecoration = class extends Decoration {
  ...
  toPMDecoration(extensionName) {
    const spec = extensionName ? { ...this.spec, key: this.key, extensionName } : { ...this.spec, key: this.key };
    return import_view6.Decoration.widget(this.pos, this.render, spec);
  }
};

// src/decorations/createWidgetDecoration.ts
...
function createWidgetDecoration(options) {
  ...
  if (cache.renderers.has(key)) {
    const previous = (_a = cache.pendingProps.get(key)) != null ? _a : cache.props.get(key);
  ...
  return Decoration.Widget(pos, render, {
    ...
    destroy: (rendererElement) => {
      ...
      try {
        (_a2 = cache.renderers.get(key)) == null ? void 0 : _a2.destroy();
        cache.renderers.delete(key);
        cache.props.delete(key);
        cache.pendingProps.delete(key);
      } finally {
        destroy == null ? void 0 : destroy(rendererElement);
      }
    }
  });
}
...
var import_transform11 = require("@tiptap/pm/transform");
function wrappingInputRule(config) {
  return new InputRule({
    ...
    handler: ({ state, range, match, chain }) => {
      ...
      const wrapping = blockRange && (0, import_transform11.findWrapping)(blockRange, config.type, attributes);
      if (!wrapping) {
        return null;
      }
      ...
      if (before && before.type === config.type && (0, import_transform11.canJoin)(tr.doc, range.from - 1) && (!config.joinPredicate || config.joinPredicate(match, before))) {
...
// src/NodeView.ts
var import_state28 = require("@tiptap/pm/state");
var NodeView = class {
  constructor(component, props, options) {
  ...
  onDragStart(event) {
    ...
    const selection = import_state28.NodeSelection.create(view.state.doc, pos);
  ...
  stopEvent(event) {
    ...
    const isSelectable = import_state28.NodeSelection.isSelectable(this.node);
